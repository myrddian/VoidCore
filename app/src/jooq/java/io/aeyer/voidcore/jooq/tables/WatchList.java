...
public class WatchList extends TableImpl<WatchListRecord> {
    ...
    /**
     * The column <code>public.watch_list.watcher_id</code>.
     */
    public final TableField<WatchListRecord, Long> WATCHER_ID = createField(DSL.name("watcher_id"), SQLDataType.BIGINT.nullable(false), this, "");

    /**
     * The column <code>public.watch_list.watched_id</code>.
     */
    public final TableField<WatchListRecord, Long> WATCHED_ID = createField(DSL.name("watched_id"), SQLDataType.BIGINT.nullable(false), this, "");

    /**
     * The column <code>public.watch_list.watched_at</code>.
     */
    public final TableField<WatchListRecord, OffsetDateTime> WATCHED_AT = createField(DSL.name("watched_at"), SQLDataType.TIMESTAMPWITHTIMEZONE(6).nullable(false).defaultValue(DSL.field(DSL.raw("now()"), SQLDataType.TIMESTAMPWITHTIMEZONE)), this, "");
    ...
    public List<Index> getIndexes() {
        return Arrays.asList(Indexes.IDX_WATCH_LIST_WATCHED);
    }
    ...
    @Override
    public List<ForeignKey<WatchListRecord, ?>> getReferences() {
        return Arrays.asList(Keys.WATCH_LIST__WATCH_LIST_WATCHED_ID_FKEY, Keys.WATCH_LIST__WATCH_LIST_WATCHER_ID_FKEY);
    }

    private transient UsersPath _watchListWatchedIdFkey;

    /**
     * Get the implicit join path to the <code>public.users</code> table, via
     * the <code>watch_list_watched_id_fkey</code> key.
     */
    public UsersPath watchListWatchedIdFkey() {
        if (_watchListWatchedIdFkey == null)
            _watchListWatchedIdFkey = new UsersPath(this, Keys.WATCH_LIST__WATCH_LIST_WATCHED_ID_FKEY, null);

        return _watchListWatchedIdFkey;
    }

    private transient UsersPath _watchListWatcherIdFkey;

    /**
     * Get the implicit join path to the <code>public.users</code> table, via
     * the <code>watch_list_watcher_id_fkey</code> key.
     */
    public UsersPath watchListWatcherIdFkey() {
        if (_watchListWatcherIdFkey == null)
            _watchListWatcherIdFkey = new UsersPath(this, Keys.WATCH_LIST__WATCH_LIST_WATCHER_ID_FKEY, null);

        return _watchListWatcherIdFkey;
    }

    @Override
    public List<Check<WatchListRecord>> getChecks() {
        return Arrays.asList(
            Internal.createCheck(this, DSL.name("watch_list_check"), "((watcher_id <> watched_id))", true)
        );
    }
