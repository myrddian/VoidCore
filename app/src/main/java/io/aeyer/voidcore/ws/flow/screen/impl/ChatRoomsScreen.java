...
@ScreenComponent
public class ChatRoomsScreen implements Screen {

    private final ChatRepository repo;
    private final AclService acl;

    public ChatRoomsScreen(ChatRepository repo, AclService acl) {
        this.repo = repo;
        this.acl = acl;
    }

    @Override public Phase phase() { return Phase.CHAT; }
    @Override public String name() { return "chat-rooms"; }

    @Override
    public List<String> topics(BbsContext ctx) {
        return ScreenFeatureGate.withTopic(List.of(io.aeyer.voidcore.ws.flow.view.ChatView.ROOMS_TOPIC));
    }
    ...
    public Transition onEnter(BbsContext ctx) {
        ...
        ctx.persistCurrentScreen("{\"kind\":\"chat_rooms\"}");
        List<ChatRoom> rooms = roomsFor(ctx);

        ArrayList<Row> rows = new ArrayList<>();
        rows.add(Frames.colored(0, "  == CHAT ROOMS ==", "bright_yellow"));
        rows.add(Frames.blank(1));
        if (rooms.isEmpty()) {
            rows.add(Frames.colored(2, "  (no rooms available)", "dark_grey"));
        } else {
            for (int i = 0; i < rooms.size(); i++) {
                ...
                rows.add(Frames.row(2 + i,
                        ...
                        Frames.span("] ", "grey"),
                        Frames.span("#" + room.slug(), "bright_cyan", true),
                        Frames.span("  " + room.label(), "default"),
                        room.privateRoom()
                                ? Frames.span("  private", "dark_grey")
                                : Frames.span("  public", "dark_grey")));
            }
        }
        int footerRow = rooms.isEmpty() ? 4 : 3 + rooms.size();
        rows.add(Frames.blank(footerRow));
        rows.add(Frames.row(footerRow + 1,
                Frames.span("  Pick a room number, or [", "grey"),
                Frames.span("M", "bright_yellow", true),
                Frames.span("] messages, [", "grey"),
                Frames.span("Q", "bright_yellow", true),
                Frames.span("] to return.", "grey")));
        rows.add(Frames.row(footerRow + 2,
                Frames.span("  In-room commands: /msg <handle> [text], /reply [text]", "dark_grey")));
        ctx.send(Frames.update("main", 51, rows));

        StringBuilder validKeys = new StringBuilder("MQ");
        for (int i = 0; i < rooms.size(); i++) validKeys.append(i + 1);
        ctx.send(new InputPrompt("keystroke", "chat rooms:", null, validKeys.toString(), null));
        return Transition.None.INSTANCE;
    }
    ...
    private List<ChatRoom> roomsFor(BbsContext ctx) {
        return repo.listActiveRooms().stream()
