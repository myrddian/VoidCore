...
@ScreenComponent
public class SysopChatRoomUserScreen implements Screen {
    ...
    public SysopChatRoomUserScreen(ChatRepository chat,
    ...
    @Override public Phase phase() { return Phase.SYSOP_CHAT_ROOM_USER; }
    @Override public String name() { return "sysop-chat-room-user"; }

    @Override
    public Transition onEnter(BbsContext ctx) {
        if (!ctx.isSysop()) { ctx.pop(); return Transition.None.INSTANCE; }
        String roomSlug = ctx.session().selectedSysopSlug();
        Long userId = ctx.session().selectedSysopResourceId();
        if (roomSlug == null || userId == null) { ctx.pop(); return Transition.None.INSTANCE; }
        ChatRoom room = chat.findRoomBySlug(roomSlug).orElse(null);
        var user = users.findById(userId).orElse(null);
        if (room == null || user == null) { ctx.pop(); return Transition.None.INSTANCE; }
        ...
        ctx.send(Frames.update("main", 78, Frames.textRows(java.util.List.of(
                ...
                "  [K] kick from room",
                "",
                "  removing a direct grant does not remove role-derived access.",
                "",
                "  [Q] back"
        ), "default")));
    ...
    public Transition onKey(BbsContext ctx, String key) {
        ...
        String roomSlug = ctx.session().selectedSysopSlug();
        Long userId = ctx.session().selectedSysopResourceId();
        if (roomSlug == null || userId == null) return Transition.None.INSTANCE;
        ChatRoom room = chat.findRoomBySlug(roomSlug).orElse(null);
    ...
    private void publishRoomRefresh(BbsContext ctx, ChatRoom room, long userId, String note) {
        ...
        ctx.audit("update_room_user_acl",
